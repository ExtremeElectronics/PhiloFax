...
int camera_init(struct camera *camera, struct camera_platform_config *params)
{
	...
	// Note: Frame rate is (no longer) ignored 
//	status = OV7670_begin(&camera->driver_host, OV7670_COLOR_YUV, OV7670_SIZE_DIV8, 0.0);
	status = OV7670_begin(&camera->driver_host, OV7670_COLOR_RGB, OV7670_SIZE_DIV2, 10);
	...
	printf("Claimed %i DMA channels for Camera\n",CAMERA_MAX_N_PLANES);
           
	camera_pio_init(camera);

	// TODO: Set an initial frame config here?
//	camera_configure(camera, FORMAT_RGB565, CAMERA_WIDTH_DIV2, CAMERA_HEIGHT_DIV2);
...
static const pio_program_t *camera_get_pixel_loop(uint32_t format)
{
	...
	case FORMAT_RGB565:
		...
		return &pixel_loop_rgb565_program;
//		return &pixel_loop_yuyv_program;
	case FORMAT_YUV422:
...
int camera_configure(struct camera *camera, uint32_t format, uint16_t width, uint16_t height)
{
	...
	// now div2!!!
	if (width != CAMERA_WIDTH_DIV2 || height != CAMERA_HEIGHT_DIV2) {
...
//	OV7670_set_size(platform, OV7670_SIZE_DIV8); //60x80
	OV7670_set_size(platform, OV7670_SIZE_DIV2);//320x240
...
//        printf("format %i, width %i, height %i\n",buf->format,buf->width,buf->height);
...
{
	return camera_do_frame(camera, into, NULL, NULL, allow_reconfigure, false);
}
